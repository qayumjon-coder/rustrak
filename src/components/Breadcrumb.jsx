...
export default function Breadcrumb() {
  ...
  return <nav className="text-sm text-light-gray py-3 pb-2.5 mb-6">
      <Link to="/" className="hover:text-yellow-500">{t("glavnaya")}</Link>
      ...
      return <span key={to}>
            {" "}/{" "}
            {isLast ? <span className="text-gray-700">{label}</span> : <Link to={to} className="hover:text-yellow-500">{label}</Link>}
          </span>;
    })}
    </nav>;
}