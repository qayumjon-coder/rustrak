  ...
  return <section className="pt-35 mb-20">
      <div className="container">
        ...
        <div className="flex flex-col gap-0">
          {partners.map((p, i) => <div key={i} className="border-b border-gray-200 py-6">
              <h2 className="text-lg font-semibold mb-2">{p.name}</h2>
              {p.description && p.description !== p.name && <p className="text-gray-700 text-sm leading-relaxed mb-2">
                  {p.description}
                </p>}
              {p.site && <a href={p.site} target="_blank" rel="noreferrer" className="text-sm text-yellow hover:underline">
                  {p.site}
                </a>}
            </div>)}
        </div>
