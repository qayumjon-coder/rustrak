...
const SwiperAbout = () => {
  ...
  return <div>
      <div className="flex items-center justify-between mb-6">
        <p className="text-[22px] max-w-162.25">{t("avtomobilnyy_zavod_rustrak_yav")}</p>
      ...
      amount: 0.2
        ...
        "--swiper-navigation-top-offset": "50%"
          ...
          {aboutSliderData.map(item => <SwiperSlide className="swiperSlide" key={item.id}>
              <motion.div variants={fadeUp} className="flex! w-full! px-4.5 pt-11.25 pb-5 flex-col h-85! border rounded-md border-light-gray/20 transition ease duration-200"> <div className="flex self-start mb-5">
                  <img src={item.icon} alt={"Truck Image " + item.id} className="self-start" />
                </div>
                <div className="py-5">
                  <h3 title={item.title?.[language] || item.title?.ru || item.title} className="text-xl w-auto text-[24px] font-medium mb-3.25">
                    {item.title?.[language] || item.title?.ru || item.title}
                  </h3>
                  <p className="">{item.text?.[language] || item.text?.ru || item.text}</p>
                </div>
